...
typedef struct tagAacENC {
    tagAacENC(void)
            : hEncoder(NULL), pOutput(NULL), pPCM(NULL), nInputSamples(0), nMaxOutputBytes(0),
              nPcmSize(0), nPcmALen(0) {
    }
...
void *aac_encoder_open(unsigned char ucAudioChannel, unsigned int u32AudioSamplerate,
    ...
    pEnc->hEncoder = faacEncOpen(u32AudioSamplerate, ucAudioChannel, &nInputSamples,
                                 &nMaxOutputBytes);
...
int aac_encoder_encode_frame(void *pHandle, unsigned char *inbuf, unsigned int inlen,
    ...
    if (pHandle != NULL) {
        ...
        if (pEnc->nPcmALen + inlen < pEnc->nPcmSize) {
            ...
            int nRet = faacEncEncode(pEnc->hEncoder, (int *) pEnc->pPCM, pEnc->nInputSamples,
                                     pEnc->pOutput, pEnc->nMaxOutputBytes);
            if (nRet > 0) {
                memcpy(outbuf, pEnc->pOutput, nRet);
                *outlen = (unsigned int) nRet;
            }
            memcpy(pEnc->pPCM, inbuf + (pEnc->nPcmSize - pEnc->nPcmALen),
                   inlen - (pEnc->nPcmSize - pEnc->nPcmALen));
