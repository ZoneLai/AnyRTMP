...
PlyBuffer::PlyBuffer(PlyBufferCallback &callback, rtc::Thread *worker)
        : callback_(callback), worker_thread_(NULL), got_audio_(false),
          cache_time_(1000)    // default 1000ms(1s)
        , cache_delta_(1), buf_cache_time_(0), ply_status_(PS_Fast), sys_fast_video_time_(0),
          rtmp_fast_video_time_(0), rtmp_cache_time_(0), play_cur_time_(0) {
    ASSERT(worker != NULL);
    ...
    if (ply_status_ == PS_Fast) {
        ...
        if (rtmp_cache_time_ <= rtc::Time()) {
            uint32_t media_buf_time = 0;
            {
                ...
                if (lst_audio_buffer_.size() > 0) {
                    media_buf_time =
                            lst_audio_buffer_.back()->_dts - lst_audio_buffer_.front()->_dts;
                }
            }
            if (media_buf_time == 0 && !got_audio_) {
                rtc::CritScope cs(&cs_list_video_);
                if (lst_video_buffer_.size() > 0) {
                    media_buf_time =
                            lst_video_buffer_.back()->_dts - lst_video_buffer_.front()->_dts;
                }
            }
