...
namespace webrtc {
    ...
    A_AACEncoder::A_AACEncoder(AVCodecCallback &callback)
            : callback_(callback), encoder_(nullptr), muted_(false), encoded_(false),
              audio_record_sample_hz_(44100), audio_record_channels_(2) {
        if (!running_) {
            running_ = true;
        }
    }
    ...
    int A_AACEncoder::Encode(const void *audioSamples, const size_t nSamples,
        ...
        if (encoder_) {
            ...
            if (muted_) {// mute audio
                memset((uint8_t *) audioSamples, 0, nSamples * nBytesPerSample * nChannels);
            }
            status = aac_encoder_encode_frame(encoder_, (uint8_t *) audioSamples,
                                              nSamples * nBytesPerSample * nChannels, encoded,
                                              &outlen);
            if (outlen > 0) {
    ...
    void A_AACEncoder::OnData(const Data &audio) {
        rtc::CritScope cs(&buffer_critsect_);
        if (encoded_) {
            if (audio_record_sample_hz_ != audio.sample_rate ||
                audio.channels != audio_record_channels_) {
                int16_t temp_output[kMaxDataSizeSamples];
                int samples_per_channel_int = resampler_record_.Resample10Msec(
                        (int16_t *) audio.data, audio.sample_rate * audio.channels,
                        audio_record_sample_hz_ * audio_record_channels_, 1, kMaxDataSizeSamples,
                        temp_output);
                Encode(temp_output, audio_record_sample_hz_ / 100, 2, audio_record_channels_,
...
//* V_H264Encoder
    V_H264Encoder::V_H264Encoder(AVCodecCallback &callback)
            : callback_(callback), need_keyframe_(true), running_(false), encoded_(false),
              video_bitrate_(768), delay_ms_(0), adjust_v_bitrate_(0),
              render_buffers_(new VideoRenderFrames(0)), video_encoder_factory_(NULL),
              encoder_(NULL) {
    ...
    void V_H264Encoder::CreateVideoEncoder() {
        ...
        if (extern_encoder != NULL) {// Try to use encoder use H/W
        ...
        if (encoder_ == NULL) {// Use software codec
...
//* For Thread
    void V_H264Encoder::Run() {
        while (running_) {
            ...
            if (frame_to_render) {
                if (h264_.width != frame_to_render->width() ||
    ...
    void V_H264Encoder::OnFrame(const cricket::VideoFrame &frame) {
        ...
        if (encoded_) {
            webrtc::VideoFrame video_frame(frame.video_frame_buffer(), 0, rtc::TimeMillis() + 150,
                                           frame.rotation());
    ...
    int32_t V_H264Encoder::Encoded(const EncodedImage &encoded_image,
        ...
        callback_.OnEncodeDataCallback(false, encoded_image._buffer, encoded_image._length,
                                       rtc::Time());
        return 0;
    }
