...
RtmpHosterImpl::RtmpHosterImpl(RTMPHosterEvent &callback)
        : callback_(callback), worker_thread_(&webrtc::AnyRtmpCore::Inst()),
          av_rtmp_started_(false), av_rtmp_streamer_(NULL), v_width_(640), v_height_(480),
          video_render_(NULL) {
...
void RtmpHosterImpl::SetVideoCapturer(void *handle) {
    ...
    if (handle != NULL) {
        ...
        worker_thread_->Invoke<void>(RTC_FROM_HERE,
                                     rtc::Bind(&RtmpHosterImpl::AddVideoCapturer_w, this,
                                               capturer));
    } else {
        worker_thread_->Invoke<void>(RTC_FROM_HERE,
                                     rtc::Bind(&RtmpHosterImpl::RemoveAVideoCapturer_w, this));
        SetVideoRender(NULL);
    }
}
...
void RtmpHosterImpl::StopRtmpStream() {
    if (av_rtmp_started_) {
        ...
        video_filter_.VBroadcaster().RemoveSink(
                ((webrtc::AnyRtmpStreamerImpl *) av_rtmp_streamer_)->GetVideoSink());
...
void RtmpHosterImpl::AddVideoCapturer_w(void *handle) {
    ...
    if (video_capturer_ != nullptr) {
...

        {
            ...
            highest_asked_format.Construct(req_w_, req_h_, cricket::VideoFormat::FpsToInterval(30),
                                           cricket::FourCC::FOURCC_NV12);
...
void RtmpHosterImpl::OnRecordAudio(const void *audioSamples, const size_t nSamples,
    ...
    webrtc::AudioSinkInterface::Data audio((int16_t *) audioSamples, nSamples, samplesPerSec,
                                           nChannels, totalDelayMS);
