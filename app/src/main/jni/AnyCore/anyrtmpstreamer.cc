...
namespace webrtc {
    AnyRtmpStreamerImpl::AnyRtmpStreamerImpl(AnyRtmpstreamerEvent &callback)
            : callback_(callback), rtmp_connected_(false), auto_adjust_bit_(false),
              a_aac_encoder_(NULL), a_muted_(false), a_enabled_(false), a_sample_hz_(44100),
              a_channels_(1), v_h264_encoder_(NULL), v_width(640), v_height(480), v_framerate_(20),
              v_bitrate_(768), av_rtmp_(NULL) {
        AnyRtmpCore::Inst();
    ...
    void AnyRtmpStreamerImpl::OnEncodeDataCallback(bool audio, uint8_t *p, uint32_t length,
                                                   uint32_t ts) {
        if (audio) {
