...
AnyRtmpPull::AnyRtmpPull(AnyRtmpPullCallback &callback, const std::string &url)
        : callback_(callback), srs_codec_(NULL), running_(false), connected_(false), retry_ct_(0),
          rtmp_status_(RS_PLY_Init), rtmp_(NULL), audio_payload_(NULL), video_payload_(NULL) {
    str_url_ = url;
...
int AnyRtmpPull::GotVideoSample(u_int32_t timestamp, SrsCodecSample *sample) {
    ...
    if (sample->has_idr) {
        ...
        if (srs_codec_->sequenceParameterSetLength > 0) {
            ...
            // sps
            video_payload_->append(srs_codec_->sequenceParameterSetNALUnit,
                                   srs_codec_->sequenceParameterSetLength);
        }
        // cont nalu header before pps.
        if (srs_codec_->pictureParameterSetLength > 0) {
            video_payload_->append((const char *) fresh_nalu_header, 4);
            // pps
            video_payload_->append(srs_codec_->pictureParameterSetNALUnit,
                                   srs_codec_->pictureParameterSetLength);
        }
    }
    ...
    if (video_payload_->_data_len != 0) {
        callback_.OnRtmpullH264Data((uint8_t *) video_payload_->_data, video_payload_->_data_len,
                                    timestamp);
    }
...
int AnyRtmpPull::GotAudioSample(u_int32_t timestamp, SrsCodecSample *sample) {
    ...
    for (int i = 0; i < sample->nb_sample_units; i++) {
        ...
        */
        ...
        callback_.OnRtmpullAACData((uint8_t *) audio_payload_->_data, audio_payload_->_data_len,
                                   timestamp);
