...
int bs_read1(bs_t *s) {
    ...
    if (s->p < s->p_end) {
        ...
        i_result = (*s->p >> s->i_left) &
                   0x01;//��Ҫ���ı����Ƶ���ǰ�ֽ����ң�Ȼ����0x01:00000001�����߼����������ΪҪ����ֻ��һ�����أ�������ز���0����1����0000 0001��λ��Ϳ��Ե�֪�����
...
PlyDecoder::PlyDecoder()
        : running_(false), playing_(false), h264_decoder_(NULL), video_render_(NULL),
          aac_decoder_(NULL), a_cache_len_(0), aac_sample_hz_(44100), aac_channels_(2),
...
void PlyDecoder::AddAACData(const uint8_t *pdata, int len, uint32_t ts) {
    if (ply_buffer_) {
        if (aac_decoder_ == NULL) {
            aac_decoder_ = aac_decoder_open((unsigned char *) pdata, len, &aac_channels_,
                                            &aac_sample_hz_);
            if (aac_channels_ == 0)
                aac_channels_ = 1;
            aac_frame_per10ms_size_ = (aac_sample_hz_ / 100) * sizeof(int16_t) * aac_channels_;
            ...
            if (aac_decoder_decode_frame(aac_decoder_, (unsigned char *) pdata, len,
                                         audio_cache_ + a_cache_len_, &outlen) > 0) {
                //printf("");
                a_cache_len_ += outlen;
                int ct = 0;
