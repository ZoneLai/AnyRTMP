...
AnyRtmpPush::AnyRtmpPush(AnyRtmpushCallback &callback, const std::string &url)
        : callback_(callback), running_(false), need_keyframe_(true), only_audio_mode_(false),
          retrys_(0), stat_time_(0), net_band_(0), sound_format_(10),
          sound_rate_(3)    // 3 = 44 kHz
        , sound_size_(1)    // 1 = 16-bit samples
        , sound_type_(1)    // 0 = Mono sound; 1 = Stereo sound
        , rtmp_status_(RS_STM_Init), rtmp_(NULL) {
    str_url_ = url;
    ...
    if (dataPtr != NULL) {
        if (dataPtr->_type == VIDEO_DATA) {
            ...
            if ((ret = srs_audio_write_raw_frame(rtmp_,
                                                 sound_format_, sound_rate_, sound_size_,
                                                 sound_type_,
                                                 (char *) dataPtr->_data, dataPtr->_dataLen,
                                                 dataPtr->_dts)) != 0) {
            ...
            int ret = srs_rtmp_write_packet(rtmp_, SRS_RTMP_TYPE_SCRIPT, dataPtr->_dts,
                                            (char *) dataPtr->_data, dataPtr->_dataLen);
            if (ret != 0) {
