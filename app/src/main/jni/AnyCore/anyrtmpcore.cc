...
namespace webrtc {
    AnyRtmpCore::AnyRtmpCore()
            : running_(false), audio_device_ptr_(NULL), audio_record_callback_(NULL),
              audio_record_sample_hz_(44100), audio_record_channels_(2),
              audio_track_callback_(NULL), audio_track_sample_hz_(44100), audio_track_channels_(2) {
        running_ = true;
        rtc::Thread::SetName("AnyRTC-RTMP-Core", this);
        rtc::Thread::Start();

        audio_device_ptr_ = AudioDeviceModuleImpl::Create(0,
        ...
        if (audio_record_callback_) {
            if (audio_record_sample_hz_ != samplesPerSec || nChannels != audio_record_channels_) {
                ...
                int samples_per_channel_int = resampler_record_.Resample10Msec(
                        (int16_t *) audioSamples, samplesPerSec * nChannels,
                        audio_record_sample_hz_ * audio_record_channels_, 1, kMaxDataSizeSamples,
                        temp_output);
                audio_record_callback_->OnRecordAudio(temp_output, audio_record_sample_hz_ / 100,
                                                      nBytesPerSample, audio_record_channels_,
                                                      audio_record_sample_hz_, totalDelayMS);
            } else {
                audio_record_callback_->OnRecordAudio(audioSamples, nSamples, nBytesPerSample,
                                                      audio_record_channels_, samplesPerSec,
                                                      totalDelayMS);
            }
        }
        return 0;
    ...
    int32_t AnyRtmpCore::NeedMorePlayData(const size_t nSamples, const size_t nBytesPerSample,
                                          const size_t nChannels,
            ...
            *ntp_time_ms = 0;
            ...
            if (readed > 0) {
                ...
                int samples_per_channel_int = resampler_track_.Resample10Msec((int16_t *) aac_buf,
                                                                              audio_track_sample_hz_ *
                                                                              audio_track_channels_,
                                                                              samplesPerSec *
                                                                              nChannels, 1,
                                                                              kMaxDataSizeSamples,
                                                                              temp_output);
                ...
                if (samples_per_channel_int > 0) {
                    memcpy(audioSamples, (char *) temp_output,
                           samples_per_channel_int * sizeof(int16_t));
