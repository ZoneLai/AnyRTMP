...
JRTMPHosterImpl::JRTMPHosterImpl(jobject javaObj)
        : m_pHoster(NULL), m_jJavaObj(NULL), m_jClass(NULL) {
    if (javaObj) {
        webrtc::AttachThreadScoped ats(webrtc_jni::GetJVM());
        m_jJavaObj = ats.env()->NewGlobalRef(javaObj);
        m_jClass = reinterpret_cast<jclass> (ats.env()->NewGlobalRef(
                ats.env()->GetObjectClass(m_jJavaObj)));
    }
...
void JRTMPHosterImpl::OnRtmpStreamReconnecting(int times) {
    ...
    {
        // Get *** callback interface method id
        jmethodID j_callJavaMId = webrtc_jni::GetMethodID(jni, m_jClass, "OnRtmpStreamReconnecting",
                                                          "(I)V");
        // Callback with params
...
void JRTMPHosterImpl::OnRtmpStreamStatus(int delayMs, int netBand) {
    ...
    {
        // Get *** callback interface method id
        jmethodID j_callJavaMId = webrtc_jni::GetMethodID(jni, m_jClass, "OnRtmpStreamStatus",
                                                          "(II)V");
        // Callback with params
...
void JRTMPHosterImpl::OnRtmpStreamFailed(int code) {
    ...
    {
        // Get *** callback interface method id
        jmethodID j_callJavaMId = webrtc_jni::GetMethodID(jni, m_jClass, "OnRtmpStreamFailed",
                                                          "(I)V");
        // Callback with params
...
void JRTMPHosterImpl::OnRtmpStreamClosed() {
    ...
    {
        // Get *** callback interface method id
        jmethodID j_callJavaMId = webrtc_jni::GetMethodID(jni, m_jClass, "OnRtmpStreamClosed",
                                                          "()V");
        // Callback with params
        jni->CallVoidMethod(m_jJavaObj, j_callJavaMId);
    }
